...
- A troca entre `Home` e `Movimientos` usa uma timeline direcional de `360ms`: a tela anterior sai por `180ms`, a próxima entra na direção de navegação após o fade-through e o indicador ativo da `Navbar` desliza durante todo o percurso. Header e fundo permanecem imóveis; o controle de compra/venda acompanha a saída ou a entrada da Home. A implementação mantém as duas telas somente durante a transição, remove as camadas temporárias ao final, cancela com segurança um retorno imediato e desativa o movimento quando `prefers-reduced-motion: reduce` estiver ativo.
