...
- Em desenvolvimento, `?previewMarketChoice=locked` força somente o estado visual indisponível de UP/DOWN para QA: preserva as cores dos botões, troca as porcentagens pelo `iconLock.svg` e mantém as ações desabilitadas. O parâmetro não atua no build de produção.
