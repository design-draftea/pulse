...
- A troca entre `Home` e `Movimientos` usa um fade-through puro de `300ms`: a tela anterior faz somente fade out e a próxima faz somente fade in, sem deslocamento nem escala. Toda navegação posiciona imediatamente as telas no topo e confirma `scrollY = 0` ao final. O indicador ativo da `Navbar` continua acompanhando o destino, enquanto header e fundo permanecem imóveis; o controle de compra/venda acompanha o mesmo fade da Home. A implementação mantém as duas telas somente durante a transição, remove as camadas temporárias ao final, cancela com segurança um retorno imediato e desativa o movimento quando `prefers-reduced-motion: reduce` estiver ativo.
