...
- `src/services/prototypeWallet.ts` mantém saldo em centavos, posições por rodada, entradas liquidadas e IDs de créditos já aplicados, sem integrar qualquer movimentação financeira real.
...
- Entradas liquidadas preservam rodada, lado, custo-base, participações, pagamento, preço objetivo e preço final no mesmo armazenamento v2. A aba `GANADAS` de `#entradas` filtra somente posições vencedoras, ordena da mais recente para a mais antiga e usa o badge `badgeGanhador.svg`. `PASADAS` reúne ganadas, perdidas e canceladas em ordem decrescente; perdidas usam `NO GANADOR`, canceladas usam `CANCELADO` e ambas mantêm a anatomia de 196px do card vencedor.
- Os chips de Entradas usam um único indicador ativo que desliza entre `ABIERTAS`, `GANADAS` e `PASADAS` em `320ms` com a mesma curva do seletor Comprar/Vender do betslip. O conteúdo faz fade-out de `110ms`, troca de estado e fade-in de `180ms`; `prefers-reduced-motion` remove os movimentos.
