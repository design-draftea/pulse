...
- Neste computador não há sessão Cloudflare para publicar o Worker. O GitHub CLI, por outro lado, está autenticado como `design-draftea` com escopos `gist`, `read:org` e `repo`, e o acesso Git por SSH permanece válido.
